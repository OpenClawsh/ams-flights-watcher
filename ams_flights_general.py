...
ALLOWED_DEPARTURE_WEEKDAYS = {1, 3, 4, 5, 6}
EXCLUDED_DESTINATIONS = {"CDG", "PAR", "ORY"}

DESTINATIONS = [
    {"code": "LHR", "name": "London", "priority": 90, "max_trip_days": 3},
...
def get_best_roundtrip(dest: dict, search_start: date, search_end: date):
    ...
    for dep in candidate_departures:
        for trip_days in allowed_trip_lengths(dest):
            ...
            if ret > search_end + timedelta(days=7): continue
            if ret.weekday() > 1: continue
            try:
                ...
                candidate = {"code": dest["code"], "name": dest["name"], "departure_date": dep.isoformat(),
                             "return_date": ret.isoformat(), "trip_days": trip_days, "price": cheapest.price,
                             "airlines": cheapest.airlines, "priority": dest["priority"]}
                if best is None or (candidate["price"], -candidate["priority"]) < (best["price"], -best["priority"]): best = candidate
...
def main():
    ...
    for dest in sorted(DESTINATIONS, key=lambda x: -x["priority"]):
        if dest["code"] in EXCLUDED_DESTINATIONS: continue
    ...
    out = {"generated_at": datetime.utcnow().isoformat() + "Z", "origin": ORIGIN,
           "search_type": "general", "constraints": {"nonstop_only": True, "round_trip": True,
           "price_cap_eur": PRICE_CAP, "default_max_trip_days": DEFAULT_MAX_TRIP_DAYS,
           "iceland_trip_days": sorted(ICELAND_TRIP_DAYS), "excluded": list(EXCLUDED_DESTINATIONS),
           "allowed_departure_weekdays": ["Tue", "Thu", "Fri", "Sat", "Sun"], "return_preference": "Mon/Tue"},
           "results": results}
    with open("results/general_flights.json", "w") as f: json.dump(out, f, indent=2)
